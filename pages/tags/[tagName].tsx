...
export const getStaticProps = async (context) => {
	...
	try {
		...
		if ((props as any).recordMap) {
			...
			if (collection) {
				const galleryView = Object.values(
					...
					if (galleryBlock?.value) {
						...
						propertyToFilterName =
							propertyToFilter?.[1]?.options.find(
								(option) =>
									normalizeTitle(option.value) ===
									filteredValue
							)?.value

						if (propertyToFilterId && filteredValue) {
							const query =
								recordMap.collection_query[collection.id]?.[
									galleryView.id
								]
							const queryResults =
								query?.collection_group_results ?? query

							if (queryResults) {
								queryResults.blockIds =
									queryResults.blockIds.filter((id) => {
